import Iframe from "../components/Iframe";
// создать проект
import lab1 from "../assets/leonova/1.pdf";

// создать страничку
import lab2 from "../assets/leonova/2.pdf";

// использование styled-components вместо CSS
import lab3 from "../assets/leonova/3.pdf";

// сделать, чтобы страница открывалась в слайдере
import lab4 from "../assets/leonova/4.pdf";

// закинуть отчеты в таблицу
import lab5 from "../assets/leonova/4.pdf";
import lab6 from "../assets/leonova/4.pdf";

import React from "react";
export default {
  l1: <Iframe src={lab1}></Iframe>,
  l2: <Iframe src={lab2}></Iframe>,
  l3: <Iframe src={lab3}></Iframe>,
  l4: <Iframe src={lab4}></Iframe>,
  l5: <Iframe src={lab5}></Iframe>,
  l6: <Iframe src={lab6}></Iframe>,
};
